...
<template>
  <div :class="$style['show-body']">
    <div :class="$style['text-content']">
      <div :class="$style.cat">
        <div :class="$style.whiskers"></div>
        <div :class="$style.face">
          <div :class="$style['ear-l']"></div>
          <div :class="$style['ear-r']"></div>
        </div>
        <div :class="$style.tag"></div>
        <div :class="$style.tail">
          <div :class="$style['tail-end']"></div>
        </div>
      </div>
    </div>
  </div>
...
## 实现思路
1. 主要是由`猫身体、猫头、猫胡须、猫耳朵、猫尾巴、猫尾部`组成。

## 代码讲解
- **`@media screen and (min-width: 600px)`**：屏幕宽度大于等于600px的时候，字体大小设置成`1.2em`，小于不受影响。
- **`em单位`**：通常用于响应式设计，一般`px`表示像素大小，`1px`是屏幕的1个物理像素。而`em`是相对单位，比如设置1.2em，如果父元素字体是10px，那么子元素字体大小是12px；
- **`border-radius`**：主要用于设置边框的圆角程度，绘画中很多圆角就是用此样式。单个参数设置4个角，两个参数设置右上角和左下角，四个参数分别设置四个角。
